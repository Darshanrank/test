...
const Applysclorship = () => {
    ...
    useEffect(() => {
        const fetchStudentData = async () => {
            try {
                ...
                const profileRes = await axios.get(`${Api}/api/update-profile/${studentId}`, {
                    headers: { Authorization: `Bearer ${token}` },
                });

                // Fetch student documents list
                const docRes = await axios.get(`${Api}/api/student-documents/${studentId}`);

                // Fetch student photo
                const photoRes = await axios.get(`${Api}/api/student-documents/${studentId}/studentPhoto`, {
                    responseType: 'blob'
                });
    ...
    const handleViewDocument = async (docType) => {
        try {
            const response = await axios.get(`${Api}/api/student-documents/${studentId}/${docType}`, {
                responseType: 'blob'
            });
