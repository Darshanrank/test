...
const arr = [img1, img2, img3, img4, img5, img6, img7, img8, img9];
    ...
    useEffect(() => {
      const incrementBoys = setInterval(() => {
        setCountBoys((prev) => (prev < 4000 ? prev + 4 : 4000));
      }, 1);
  
      const incrementGirls = setInterval(() => {
        setCountGirls((prev) => (prev < 2000 ? prev + 2 : 2000));
      }, 1);
  
      const incrementTotal = setInterval(() => {
        setCountTotal((prev) => (prev < 6000 ? prev + 6 : 6000));
      }, 1);
  ...
  return (
    ...
    <div className='hidden sm:flex h-[2.6rem]  justify-end content-center'>
        ...
        <button
            onClick={hendelbnev}
        className='flex pr-2 pl-2 text-lg font-bold mr-[0.8rem] pt-[0.2rem] border-black border-2 hover:text-white hover:bg-black rounded'>Login<CiLogin className='block h-[1.8rem] w-[1.5rem]  mt-[0.15rem] text-bold'/></button>
    </div>
    <div className='pr-[10%] pl-[10%] mt-[1%] mb-[2%]'>
        <h2 className='font-bold  text-xl pb-[1%]'>Prime Minister’s Scholarship Scheme (PMSS)</h2>
